...
import { ChatMessage, TarotReading } from '../lib/types';
...
export const useChat = (predictionId: string) => {
  ...
  const [tarotPrediction, setTarotPrediction] = useState<TarotReading | null>(null);
  ...
  const loadTarotPrediction = async () => {
    if (user) {
      try {
        const supabaseUserId = await getSupabaseUserId(user.id);
        if (supabaseUserId) {
          const prediction = await getTarotReading(supabaseUserId, predictionId);
          setTarotPrediction(prediction);
        }
      } catch (error) {
        console.error('Error loading tarot prediction:', error);
        toast({
          title: 'Error',
          description: 'Failed to load tarot prediction',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
  ...
  const handleSendMessage = async () => {
    if (input.trim() && user && tarotPrediction) {
      setIsLoading(true);
      try {
        const supabaseUserId = await getSupabaseUserId(user.id);
